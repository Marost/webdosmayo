...
/* VARIABLES */
$url_web=$web."boletin-epidemiologico";
...
$rst_cas        = mysql_query("SELECT COUNT(*) as count FROM DM_epidemiologico ORDER BY fecha_publicacion DESC", $conexion);
...
$rst_cas        = mysql_query("SELECT * FROM DM_epidemiologico ORDER BY fecha_publicacion DESC LIMIT $start, 5", $conexion);

################################################################
/*SELECCION DE AÑO*/
$rst_selectanio=mysql_query("SELECT DISTINCT fecha_anio FROM DM_epidemiologico", $conexion);
...
if($anio>0 AND $mes>0){
    $url_web=$web."boletin-epidemiologico?anio=$anio&mes=$mes";
    $nombre_fecha=nombreMes($mes)." ".$anio;
    $page = (isset($_GET['page'])) ? intval($_GET['page']) : 1;
    $rst_cas        = mysql_query("SELECT COUNT(*) as count FROM DM_epidemiologico WHERE fecha_mes='$fecha_seleccion' ORDER BY fecha_publicacion DESC", $conexion);
    $fila_cas       = mysql_fetch_assoc($rst_cas);
    $generated      = intval($fila_cas['count']);
    $pagination     = new Pagination("5", $generated, $page, $url_web."&page", 1, 0);
    $start          = $pagination->prePagination();
    $rst_cas        = mysql_query("SELECT * FROM DM_epidemiologico WHERE fecha_mes='$fecha_seleccion' ORDER BY fecha_publicacion DESC LIMIT $start, 5", $conexion);
}
...
if($buscar<>""){
    $url_web=$web."boletin-epidemiologico?buscar=$buscar";
    $page = (isset($_GET['page'])) ? intval($_GET['page']) : 1;
    $rst_cas        = mysql_query("SELECT COUNT(*) as count FROM DM_epidemiologico WHERE titulo LIKE '%$buscar%' ORDER BY fecha_publicacion DESC", $conexion);
    $fila_cas       = mysql_fetch_assoc($rst_cas);
    $generated      = intval($fila_cas['count']);
    $pagination     = new Pagination("5", $generated, $page, $url_web."&page", 1, 0);
    $start          = $pagination->prePagination();
    $rst_cas        = mysql_query("SELECT * FROM DM_epidemiologico WHERE titulo LIKE '%$buscar%' ORDER BY fecha_publicacion DESC LIMIT $start, 5", $conexion);
}
                        ...
                        <div class="nw-nota cas">
                            ...
                            <div class="contenido">
                                ...
                                <div id="busqueda_cabecera">
                                    ...
                                    <form name="busqueda" action="boletin-epidemiologico" class="search-form noframe nobtn rsmall">
                                ...
                                <table class="tabla_cas" width="710" border="0">
                                    <tbody>
                                        ...
                                        <?php while($fila_cas=mysql_fetch_array($rst_cas)){
                                            ...
                                            /*DOCUMENTOS*/
                                            $rst_cas_docs=mysql_query("SELECT * FROM DM_epidemiologico_documentos WHERE cas=$cas_id ORDER BY orden ASC;", $conexion);
                                        ?>
                                        <tr>
